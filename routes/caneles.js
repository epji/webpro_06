...
// 一覧
router.get("/", (req, res) => {
  res.render("caneles/index", { caneles });
});

// 新規追加フォーム
router.get("/new", (req, res) => {
  res.render("caneles/new");
});
...
router.get("/:id", (req, res) => {
  ...
  if (!canele) return res.status(404).send("カヌレが見つかりません。");
  res.render("caneles/detail", { canele });
});

// 新規追加 POST
router.post("/", (req, res) => {
  const newId = caneles.length ? Math.max(...caneles.map(c => c.id)) + 1 : 1;
...
router.get("/:id/edit", (req, res) => {
  ...
  const canele = caneles.find(c => c.id === id);
  if (!canele) return res.status(404).send("カヌレが見つかりません。");
  res.render("caneles/edit", { canele });
});
